...
export default function Profile(): JSX.Element | null {
  ...
  const inputs = {
    name: (
      <>
        <Input value={profileValues.given} name="given" handleChange={handleInputChange} placeholder="Nombres" />
        <Input value={profileValues.family} name="family" handleChange={handleInputChange} placeholder="Apellido" />
      </>
    ),
    gender: <Input value={profileValues.gender} name="gender" handleChange={handleInputChange} placeholder="Género" />,
    birthDate: (
      <Input
        type="date"
        name="birthDate"
    ...
    address: (
      <>
        <Input name="line1" value={profileValues.line1} handleChange={handleInputChange} placeholder="Dirección 1" />
        <Input name="line2" value={profileValues.line2} handleChange={handleInputChange} placeholder="Dirección 2" />
        <Input name="city" value={profileValues.city} handleChange={handleInputChange} placeholder="Ciudad" />
        <Input name="state" value={profileValues.state} handleChange={handleInputChange} placeholder="Provincia" />
	<Input name="postalCode" value={profileValues.postalCode} handleChange={handleInputChange} placeholder="Código Postal" />
      </>
    ),
  };
  ...
  useEffect(() => {
    if (resource) {
      const { telecom = [{ system: '', use: '', value: '' }], address = [{ line: ['', ''], city: '', state: '', postalCode: '' }] } =
        resource;
          ...
          {activeInputName === 'gender' ? (
            ...
            <Button onClick={() => setActiveInputName('gender')}>
              <div className="flex items-center text-gray-400 hover:text-emerald-700">
                ...
                <p className="text-lg">Agregar género</p>
              </div>
            </Button>
          )}
        </>
        ...
        <>
          {activeInputName === 'birthDate' ? (
            ...
            <Button onClick={() => setActiveInputName('birthDate')}>
              <div className="flex items-center text-gray-400 hover:text-emerald-700">
                <PlusIcon className="mr-2 h-5 w-5 self-center" aria-hidden="true" />
                <p className="text-lg">Agregar fecha de nacimiento</p>
              </div>
            </Button>
          )}
        </>
        ...
        <>
          {activeInputName === 'contact' ? (
            ...
            <Button onClick={() => setActiveInputName('contact')}>
              <div className="flex items-center text-gray-400 hover:text-emerald-700">
                <PlusIcon className="mr-2 h-5 w-5 self-center" aria-hidden="true" />
                <p className="text-lg">Agregar contacto</p>
              </div>
            </Button>
          )}
        </>
      ...
      body: (
        <>
          {activeInputName === 'address' ? (
            ...
            <>
              {resource.address.map(({ city, line, state, postalCode }, addressIndex) => (
		...
		{ op: 'replace', path: '/address/0/postalCode', value: profileValues.postalCode },
              ]);
            }}
          >
